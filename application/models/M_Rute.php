<?php 
	...
	*/
	class M_Rute extends CI_Model
	{
		...
		function getDataRuteP(){
			$query = $this->db->query("SELECT R.* from rute R inner join transportation T on R.transportationid = T.transportationid where T.transportation_typeid = 'T0002'");
...
function getlistBandara() {
    ...
    {
        $this->db->select('stasiunid');
        $this->db->from($this->table1);
        $this->db->order_by('stasiunid','asc');
        $query = $this->db->get();
        $result = $query->result();
